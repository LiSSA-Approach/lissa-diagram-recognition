...
class SketchRCNNOutputLayers(nn.Module):
        ...
        if len(proposals):
            ...
            gt_keypoints = cat(
                [(p.gt_keypoints.tensor
                  ...
                  else torch.Tensor([-1, -1, -1, -1], device=proposal_deltas.device))
                 for p in proposals],
                dim=0,
            )
        else:
    ...
    def arrow_loss(self,
        ...
        loss = mse(proposals_arrow_keypoints[:, 0], x_head) + \
               mse(proposals_arrow_keypoints[:, 1], y_head) + \
               mse(proposals_arrow_keypoints[:, 2], x_tail) + \
               mse(proposals_arrow_keypoints[:, 3], y_tail)
